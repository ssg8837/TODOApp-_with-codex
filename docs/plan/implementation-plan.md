...
- 기존 `TodoService.observeFiltered()`와 DAO AND 조회를 재사용한다. 필터 결과가 비었을 때만 기존 날짜별 Flow를 구독해 빈 상태를 구분하고, 결과 또는 조건 변경 시 불필요한 구독을 취소한다.
- 날짜 이동 시 필터를 유지하며 선택 Category가 삭제되면 선택지와 선택값을 함께 갱신하여 전체로 복귀한다.
